"""Analysis of kMC trajectories: sink identification, pathway extraction, and
report emission. Adapted from the framework of Barter et al. with this
...
from chemistry_lib.constants import ROOM_TEMP, KB
from chemistry_lib.reaction_questions import marcus_barrier
...
def render_species(network_loader, path):
    from chemistry_lib.network_renderer import Renderer
    renderer = Renderer()
...
def sink_report(
    ...
    for species_index in sinks_sorted:
        ...
        if pathfinding is not None:
            try:
                _pdict = pathfinding.compute_pathways(species_index)
                _lens = [len(d["pathway"]) for d in _pdict.values() if d.get("pathway")]
                if _lens:
                    _msg = str(min(_lens)) + " steps"
                else:
                    _msg = "unreachable in sampled trajectories"
            except Exception:
                _msg = "n/a"
            report_generator.emit_text("shortest pathway length: " + _msg)
